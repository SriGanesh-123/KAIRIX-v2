"""
Analyze Page for KAIRIX UI.

Executes the end-to-end 7-stage Knowledge Engineering pipeline on a selected source file,
reporting real-time progress and displaying extracted canonical knowledge in light theme without emojis.
"""
from __future__ import annotations

import streamlit as st
from ui.services.analyze_service import AnalyzeService
from ui.services.source_service import SourceService
from ui.components.metric_cards import format_metric


def render_analyze() -> None:
    """
    Renders the single-file Knowledge Engineering analysis page in light theme.
    """
    st.markdown("## Knowledge Engineering Pipeline")
    st.markdown(
        "<p style='color: #64748B; margin-top: -0.5rem;'>Run deterministic parsing, AST extraction, multi-pass LLM review, and graph reconciliation on legacy code.</p>",
        unsafe_allow_html=True,
    )

    all_files = SourceService.get_all_source_files()
    file_map = {f["file_name"]: f["file_path"] for f in all_files}

    # Pre-select target file if set from Source Explorer
    preselected_path = st.session_state.get("analyze_target_file", "")
    default_name = None
    if preselected_path:
        for name, path in file_map.items():
            if path == preselected_path:
                default_name = name
                break

    file_names = list(file_map.keys())
    default_idx = file_names.index(default_name) if default_name in file_names else 0

    col_select, col_refresh = st.columns([3, 1])
    with col_select:
        selected_file_name = st.selectbox(
            "Select Source File to Analyze:",
            options=file_names,
            index=default_idx,
            key="analyze_file_select",
        )
    with col_refresh:
        force_refresh = st.checkbox("Force Fresh LLM Run", value=False, help="Bypass local cache and re-execute full LLM pipeline.")

    selected_file_path = file_map.get(selected_file_name, "")

    # Display Pipeline Stages Overview
    st.markdown(
        """
        <div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:8px; padding:0.75rem 1rem; margin:1rem 0; box-shadow:0 1px 2px rgba(0,0,0,0.03);">
            <div style="font-size:0.78rem; font-weight:700; text-transform:uppercase; color:#64748B; margin-bottom:0.4rem;">
                7-Stage Deterministic & Agentic Pipeline
            </div>
            <div style="display:flex; flex-wrap:wrap; gap:0.4rem; font-size:0.82rem; color:#475569; font-weight:500;">
                <span>1. Classification →</span>
                <span>2. Parsing (AST) →</span>
                <span>3. Evidence Building →</span>
                <span>4. LLM Review →</span>
                <span>5. Knowledge Extraction →</span>
                <span>6. Reconciliation →</span>
                <span style="color:#0284C7; font-weight:700;">7. Canonical Package</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    )

    if st.button("Run Analysis", type="primary", use_container_width=True):
        if not selected_file_path:
            st.error("Please select a valid source file.")
            return

        with st.status(f"Analyzing {selected_file_name}...", expanded=True) as status:
            st.write("1. Source Classification: Detecting dialect and routing to parser...")
            st.write("2. Parser Execution: Extracting AST nodes, statements, and symbol tables...")
            st.write("3. Evidence Building: Compiling syntax anchors and line references...")
            st.write("4. LLM Artifact Review: Running multi-pass semantic inspection...")
            st.write("5. Knowledge Extraction: Extracting business rules and transformations...")
            st.write("6. Reconciliation: Cross-validating AST facts with LLM discoveries...")
            st.write("7. Canonical Packaging: Generating canonical JSON and Neo4j graph nodes...")

            result = AnalyzeService.run_analysis(
                file_path=selected_file_path,
                force_refresh=force_refresh,
            )

            if result.get("success"):
                status.update(label=f"Analysis completed successfully for {selected_file_name}!", state="complete", expanded=True)
                st.session_state["last_analysis_result"] = result
                SourceService.refresh_sources()
            else:
                status.update(label=f"Analysis encountered an issue: {result.get('error')}", state="error", expanded=True)

    # Display Analysis Result from current run or existing cached package
    last_res = st.session_state.get("last_analysis_result")
    pkg = SourceService.get_knowledge_package(selected_file_name)

    if (last_res and last_res.get("file_name") == selected_file_name) or pkg:
        st.markdown("<div style='margin-top:1.5rem;'></div>", unsafe_allow_html=True)
        st.markdown("### Analysis Output Summary")

        if last_res and last_res.get("file_name") == selected_file_name:
            entities_count = last_res.get("entities_count", 0)
            transformations_count = last_res.get("transformations_count", 0)
            rules_count = last_res.get("rules_count", 0)
            relationships_count = last_res.get("relationships_count", 0)
            purpose_text = last_res.get("summary") or "No purpose extracted."
            pkg_json = last_res.get("package_dict", {})
        else:
            profile = pkg.get("knowledge_profile", {})
            summary = pkg.get("summary", {})
            recon = pkg.get("reconciliation", {})
            entities_count = len(profile.get("entities", [])) or len(pkg.get("graph_nodes", []))
            transformations_count = len(profile.get("transformations", []))
            rules_count = len(summary.get("business_rules", [])) or len(profile.get("business_rules", []))
            relationships_count = len(profile.get("relationships", [])) or len(pkg.get("graph_edges", []))
            purpose_text = summary.get("purpose") or summary.get("high_level_narrative") or "Knowledge profile available on disk."
            pkg_json = pkg

        col_r1, col_r2, col_r3, col_r4 = st.columns(4)
        with col_r1:
            st.metric("Entities", format_metric(entities_count))
        with col_r2:
            st.metric("Transformations", format_metric(transformations_count))
        with col_r3:
            st.metric("Business Rules", format_metric(rules_count))
        with col_r4:
            st.metric("Relationships", format_metric(relationships_count))

        st.markdown(
            f"""
            <div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:8px; padding:1rem; margin-top:1rem; box-shadow:0 1px 2px rgba(0,0,0,0.03);">
                <div style="font-weight:700; color:#0284C7; margin-bottom:0.3rem;">Purpose Narrative:</div>
                <div style="color:#1E293B; font-size:0.95rem; line-height:1.5;">{purpose_text}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

        with st.expander("View Generated Canonical JSON Package", expanded=False):
            st.json(pkg_json)
